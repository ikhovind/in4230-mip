...
void build_eth_header(eth_header* header, const uint8_t dest_address[ETH_ADDR_LEN], const uint8_t source_address[ETH_ADDR_LEN], const uint16_t ethertype) {
...
void build_eth_pdu(eth_pdu* eth_pdu, const eth_header* header, const mip_pdu* mip_pdu) {
    eth_pdu->header = *header;
    eth_pdu->mip_pdu = *mip_pdu;
}

void serialize_eth_pdu(uint8_t* target, const eth_pdu* eth_pdu) {
...
void print_mac_address(const char *label, const uint8_t address[6], int indent) {
